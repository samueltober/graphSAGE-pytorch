...
class UnsupervisedLoss(object):
    ...
    def get_loss_sage(self, embeddings, nodes):
        ...
        assert False not in [
            nodes[i] == self.unique_nodes_batch[i] for i in range(len(nodes))
        ]
        ...
        for node in self.node_positive_pairs:
            ...
            nodes_score.append(
                torch.max(
                    ...
                    neg_score - pos_score + self.MARGIN,
                ).view(1, -1)
            )
    ...
    def extend_nodes(self, nodes, num_neg=6):
        ...
        self.unique_nodes_batch = list(
            set([i for x in self.positive_pairs for i in x])
    ...
    def get_negtive_nodes(self, nodes, num_neg):
        for node in nodes:
            ...
            neg_samples = (
                random.sample(far_nodes, num_neg)
                if num_neg < len(far_nodes)
                else far_nodes
            )
...
class GraphSage(nn.Module):
    ...
    def _get_unique_neighs_list(self, nodes, num_sample=10):
        ...
        if not num_sample is None:
            ...
            samp_neighs = [
                _set(_sample(to_neigh, num_sample))
        ...
        samp_neighs = [
            samp_neigh | {nodes[i]} for i, samp_neigh in enumerate(samp_neighs)
        ]
    ...
    def aggregate(self, nodes, pre_hidden_embs, pre_neighs):
        ...
        if not self.gcn:
            samp_neighs = [
                (samp_neighs[i] - {nodes[i]}) for i in range(len(samp_neighs))
            ]
